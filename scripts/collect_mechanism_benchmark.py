    ...
    with (out/'collector.lock').open('a') as lock:
        ...
        while True:
            try:
                ...
                if q['state']=='complete':
                    r=remote('report/report_status.json')
                    if a.supplement:assert r['total_training_configurations']==213 and r['total_views']==231 and r['new_trainings']==27
                    else:assert r['complete_trials']==r['total_trials']==186 and r['seeds']==[3416,3417,3418]
                    subprocess.run(['rsync','-az','-e','ssh -o BatchMode=yes -o ConnectTimeout=10','ws02:'+a.root+'/report/',str(out)+'/'],check=True,timeout=300)
                    d=json.loads((out/'results.json').read_text());assert len(d['rows'])==(231 if a.supplement else 186) and not d['test_used']
                    status('collected_pending_pdf_authoring_and_visual_review',complete_trials=213 if a.supplement else 186,evaluation_views=231 if a.supplement else 186);return
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--root',required=True);p.add_argument('--destination',required=True);p.add_argument('--supplement',action='store_true');run(p.parse_args())
