"""Integer-configured, native-node-preserving many-participant Rhythm Bridge."""
from dataclasses import dataclass
import math
import torch
from torch import nn
from .projector import Projector, positive_integer
...
class BridgeExchange(nn.Module):
    def __init__(self, specs, *, M, S, H, mode='radon'):
        super().__init__()
        for name, value, minimum in [('M', M, 1), ('S', S, 2), ('H', H, 1)]:
            positive_integer(value, name, minimum)
        if mode not in ('radon', 'self', 'pooled'):
        ...
        self.mode, self.M, self.S, self.H = mode, M, S, H
        self.projectors = nn.ModuleList([] if mode == 'pooled' else [Projector(s.shape, M, S) for s in specs])
        widths = [s.channels if mode == 'pooled' else s.channels*M for s in specs]
        self.compress = nn.ModuleList([nn.Conv1d(w, H, 1, bias=False) for w in widths])
        self.expand = nn.ModuleList([nn.Conv1d(H, w, 1, bias=False) for w in widths])
        self.mixer = LinearMixer([H]*len(specs), 1 if mode == 'pooled' else 3, mode == 'self')
        self.metadata = {'mode': mode, 'M': M, 'S': S, 'H': H, 'participants': [
            {'key': s.key, 'channels': s.channels, 'shape': list(s.shape), 'projected_channels': w,
             'H': H, 'achieved_width_ratio': H/w,
...
def attach_group(node, edge, specs, inputs, prefix, *, M, S, H, mode='radon'):
    ...
    exchange = BridgeExchange(specs, M=M, S=S, H=H, mode=mode)
...
def attach_to_nodes(builder, node_names, *, prefix, M, S, H, samples=None, mode='radon'):
    ...
    result,meta=attach_group(builder.node,builder.edge,specs,inputs,prefix,M=M,S=S,H=H,mode=mode)
