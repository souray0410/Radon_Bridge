...
三个项目的应用/开发根均为各自 Git 检出根。LOOK 从本次准备提交起将原 home 内的代码提升到仓库根，与 Radon_Bridge 一致；旧发布目录与旧提交保留原布局以供复现。
...
MHD_Project 是框架的唯一上游；LOOK 和 Radon_Bridge 是两个独立应用，彼此不作为代码或虚拟环境依赖。MHD 当前开发 V5，但本次两应用明确固定 V4。`framework.lock.json` 记录上游提交、API 代次及每个实际使用文件的 SHA256。执行 `python workspace/check_framework.py --project-root . --upstream <MHD检出>` 核验。

LOOK 与 Radon_Bridge 统一使用 `third_party/MHD_Project` 固定提交子模块，Python 均从 `V4` 导入，两者环境单独安装同一上游代码。LOOK 本次删除重复的内置源码副本，保留旧版本于历史 Git 提交/部署；新代码不添加旧导入别名。框架类和算法字节不变，导入路径改变需通过输出/梯度/状态重载验收。

同一时间戳既是各仓库准备分支 `2026_09_09_10_30_34`，也是各自部署目录名；三个仓库提交哈希独立记录。后续准备改动先推时间戳分支，main 保持已审核状态；开始正式实验后锁定不可变提交，后续修复独立 attempt。

2026-09-09 用户授权停止 ws02 LOOK，已完成证据保留；本次 Ibex 准备不自动续跑旧队列。新的数据划分、研究协议及GPU任务尚待确定。共享原始影像及表型CSV，两个应用各自生成有协议与摘要的派生缓存，不直接把完整原始CSV当已验收的训练标签。
