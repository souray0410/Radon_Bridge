...
def check(project_root, upstream=None):
    ...
    if lock['schema'] not in ('mhd_source_lock_v1', 'mhd_source_lock_v2') or not re.fullmatch(r'V[1-9][0-9]*', lock['api_version']):
    ...
    for row in lock['files']:
        if lock['schema']=='mhd_source_lock_v1' and Path(row['upstream_path']).parts[0]!=lock['api_version']:
            raise ValueError('Locked file belongs to another API')
        if lock['schema']=='mhd_source_lock_v2' and (not row['upstream_path'].startswith('src/mhd_framework/') or not row['module'].startswith('mhd_framework')):raise ValueError('Unexpected packaged framework module')
