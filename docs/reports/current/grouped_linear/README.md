...
本页固定SVD r32/M32/S64/k3，比较G=1/2/4/8/16；每个G均配同G普通线性重采样。另见[项目总览与覆盖](../README.md)、[已完成24项线性分解](../factorized/README.md)、[覆盖与后续机制](../coverage.md)。
...
完整分组匹配包：已接受。右侧包内 audit 与左侧独立复核均通过；接受范围仅为本 WS02 单种子开发集包，不代表 Ibex、test 或全项目接受。

|方法|CFP分支F1|OCT分支F1|分支均值F1|最佳/停止轮|来源|
|---|---:|---:|---:|---|---|
|G=1 Radon|72.30%|71.28%|71.79%|11/17|G=1严格复用已接受核心结果|
|G=1 普通通信|69.59%|65.46%|67.53%|9/15|G=1严格复用已接受核心结果|
|G=2 Radon|68.71%|70.26%|69.49%|6/12|新增单种子3416匹配执行|
|G=2 普通通信|66.89%|65.14%|66.01%|0/8|新增单种子3416匹配执行|
|G=4 Radon|70.60%|66.74%|68.67%|8/14|新增单种子3416匹配执行|
|G=4 普通通信|66.89%|65.14%|66.01%|0/8|新增单种子3416匹配执行|
|G=8 Radon|67.12%|65.48%|66.30%|6/12|新增单种子3416匹配执行|
|G=8 普通通信|66.89%|65.14%|66.01%|0/8|新增单种子3416匹配执行|
|G=16 Radon|67.89%|64.52%|66.20%|8/14|新增单种子3416匹配执行|
|G=16 普通通信|66.89%|65.14%|66.01%|0/8|新增单种子3416匹配执行|
...
G表示桥内分组数：G=1是稠密混合；G越大，每组越小，桥内直接连接比例与mixer参数量都约按1/G下降。因此跨G点估计同时改变通信拓扑和参数量；只有同一G下Radon与普通线性重采样是参数匹配的几何对照。
...
左侧独立复核确认：89个源码文件与部署提交一致，54份科研资产哈希通过，296人ID/标签顺序与分支F1独立重算一致，10,000次五项普通及同时区间重算一致。公开仓库只保留摘要、逻辑资产引用与校验值，不上传参与者预测、权重或凭据。
