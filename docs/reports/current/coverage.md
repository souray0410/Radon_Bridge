...
|SVD与QR/可学习通道是否不同|[有限六项匹配包](channel_compression/README.md)|原SVD两项复用，其余四项新预检/训练；中心化方向另待验收|
...
|大队列六臂/三疾病/三架构|有限162位置；源状态保留最近运行核验时间|父模型、实际pair/runtime/恢复/逐臂验收尚未齐；本次没有重新核验Ibex运行，不把旧paused或pending说成当前训练|
