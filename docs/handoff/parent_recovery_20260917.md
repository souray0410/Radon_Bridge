...
该补充在Ibex独立源码通过31项针对测试及98项完整单元测试，包括更换spec/硬件/断点、证据损坏、资源不足和科学companion拒绝。**尚缺实际GPU新断点重验与耗时测量，故未换正在运行的管理器或科学worker，不能称已经减少了1–2小时。** 等安全资源准入完成该项后才能激活。

完整测试证据：operations `radon_profile_reuse_1bfec97/unit_tests.log`，98 passed，14项既有绘图库弃用警告。初次测试包缺Git子模块元数据和Mac归档旁文件导致结构检查失败，完整打包/恢复真实V4 Git元数据后通过；未通过删除测试或放宽校验解决。
