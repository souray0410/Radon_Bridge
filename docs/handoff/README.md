## 2026-09-25 02:12 UTC：原父模型已在WS02正式V5续训

macular DenseNet3D 原run从26284次更新迁移后续训到26286次，epoch8；WS02 GPU0归R&B，GPU1健康LOOK未动。完整状态和历史best保留，原Slurm终结证据与领取记录在同一把锁下显式释放/重领，原数据和旧run源不改写。实际V5两步连续更新与新进程恢复逐项完全相等，仅排除墙钟seconds。

6小时安全断点接续的单一监督进程已部署，首次正式训练真实推进；实际6小时轮次尚未到达，不称该分支已验。父模型尚未选优接受，完整六臂仍待接通；Ibex对应V5任务继续排队。[实际回执](../acceptance/ws02_actual_parent_running_20260925.json)。下方较早“正式父模型未部署”已由本条取代，其余历史仅按原日期解读。

## 2026-09-24 21:36 UTC：完整缓存与实际 V5 恢复接受

2026-09-24 21:36UTC：WS02 70,885人/212,655数组逐一SHA接受。修复框架src导入路径和独立MONAI1.5.1依赖后，GPU0真实update1及两次新进程update2完成；另跑不中断两步，完整模型/优化器/scheduler/RNG/游标严格相等，仅排除墙钟seconds。失败记录保留、临时传输键清除。只接受工程路径，正式父模型/六臂尚未接通。

[实测回执](../acceptance/ws02_full_cache_resume_accepted_20260924.json)。下一项由主窗口继续完成正式父模型/六臂的互斥领取与接续；不把工程资格断点直接变为科学选优产物。Ibex52473855仍待卡，健康旧任务保留。
...
Ibex 正式 V5 续训 52473855 排队，原唯一 successor 负责终结与受控接续，健康旧 V4 作业保留。WS02 独立目标缓存直接复用/复制 Ibex 已处理的完整 macular 队列，38k 左右的逐数组摘要已核验；原缓存写入者及原目录未改。全量 70,885 人/212,655 数组验收后，已部署 GPU0 自动单更新资格入口；新进程恢复验收仍待接通。

下一项：完成完整缓存复制与逐数组 SHA；自动运行 GPU0 资格，核验真实恢复与下一更新，然后接通正式父模型及六臂研究；Ibex 获卡后续训原 run。
...
证据：ws02原run的`independent_final_audit.json`；[累计三臂结果](../reports/current/augmentation/README.md)。原有限队列正常结束，下一包未启动；下方为历史快照。
