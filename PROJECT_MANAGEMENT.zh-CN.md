...
- 历史分支只能使用 `YYYY_MM_DD_HH_MM_SS`，不再创建或保留 exp/实验编号分支。
  实验名称与编号放在目录、提交说明和实验索引中。常规小修复逐次提交，
  按一次可执行更新归档，不按每一行修改建分支。
...
历史 exp 分支已迁移为纯时间戳分支，映射及完整提交见
`experiments/BRANCH_ARCHIVE.json`。001/002 的补录分支采用原提交时间，
003 复用相同提交的已有时间戳归档，004 使用本次归档时间；提交内容未变。
执行来源 tag 保留以免破坏结果溯源，tag 不是分支。
...
ws 始终在最新 main 上运行，更新等待当前运行结束。所有后续部署和批次运行必须遵守该锁；它不是对绕过流程的系统级隔离。
