...
- `main`：已审阅的稳定基线。实验完成后评估是否合入；保留负结果。
- `exp/NNN-假设`：每个可解释的实验改动一个永久分支。修复、协议、代码、
  验证和结论在该分支逐次提交，不为每一行修改创建新分支。
...
部署步骤：提交并推送 GitHub → 确认 ws 工作区干净 → 获取运行锁
`/data/mengh/RadonBridge/.active.lock` → fetch 指定分支（可用 Git bundle，
无需在 ws 配置 GitHub 凭据）→ 单工作区切换到该分支并快进 → 校验 commit
与 MHD submodule → 放开部署锁 → 在同一个锁下执行实验。锁被占用则不切换。
所有后续部署和批次运行必须遵守该锁；它不是对绕过流程的系统级隔离。
