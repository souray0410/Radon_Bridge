...
def attach_communications(native,configs):
    ...
    if parallel and (len(configs)!=2 or configs[1].get('parallel_to')!=0 or configs[0].get('family') not in ('mmtm','cross_attention','cmx_frm','cmx_full') or configs[1].get('family','radon')!='radon' or configs[0]['nodes']!=configs[1]['nodes']):
    ...
    for c in configs:
        ...
        required={'nodes','M','S','rho','mode'} if family=='radon' else {'nodes','family','reduction_ratio'} if family=='mmtm' else {'nodes','family','attention_dimension','heads'} if family=='cross_attention' else {'nodes','family','alignment_tokens','heads'} if family=='cmx_full' else {'nodes','family','alignment_tokens'}
        ...
        if family not in ('radon','mmtm','cross_attention','cmx_frm','cmx_full') or not required<=set(c) or set(c)-required-optional:
