...
def attach_communications(native,configs):
    builder=native.builder;configs=[dict(c) for c in configs]
    ...
    if parallel and (len(configs)!=2 or configs[1].get('parallel_to')!=0 or configs[0].get('family') not in ('mmtm','cross_attention') or configs[1].get('family','radon')!='radon' or configs[0]['nodes']!=configs[1]['nodes']):
    ...
    for c in configs:
        ...
        required={'nodes','M','S','rho','mode'} if family=='radon' else {'nodes','family'} if family=='mmtm' else {'nodes','family','attention_dimension','heads'}
        optional={'family','compression','basis_files','cross_edges','nested_rhos','s_axis_permutation','kernel_size','r','h','bottleneck_rank','parallel_to'} if family=='radon' else {'reduction_ratio','hidden_dimension'} if family=='mmtm' else set()
    ...
    for c in configs:
        axes=native.metadata.get('channel_axes',{})
        if any(axes.get(n,1)!=1 for n in c['nodes']):c['channel_axes']={n:axes.get(n,1) for n in c['nodes']}
