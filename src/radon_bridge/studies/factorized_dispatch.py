...
import sys
import time
from radon_bridge.studies.factorized_ws import sha, write_json
...
def main():
    ...
    while True:
        ...
        for gpu in args.devices:
            ...
            cmd=[sys.executable,'-m','radon_bridge.studies.factorized_ws',mode,'--config',c['config'],'--data',q['data'],'--output',str(output)]
    ...
    if len(complete)==len(cases):
        from radon_bridge.analysis.factorized_report import report
        report(root)
